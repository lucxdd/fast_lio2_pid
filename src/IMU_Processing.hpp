...
/** 2. 将加速度测量值标准化为单位重力**/
// 这里应该是静止初始化,内部主要是对x_和P_完成了初始化，这里涉及到esikfom文件内部的知识，这里暂且先不讲
// 用于声明一个名为kf_state的对象，它是一个esekfom::esekf类的实例12。esekfom::esekf是一个模板类，它实现了一个嵌入流形的扩展卡尔曼滤波器（ESEKF）31。它的模板参数分别是状态变量、过程噪声维度和输入变量12。
// 在这个例子中，状态变量是state_ikfom，过程噪声维度是12，输入变量是input_ikfom。
// 这段代码是用于创建一个机器人状态估计器的对象，它可以根据IMU的测量值来更新机器人的状态。
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  // 0到2行和列：位置(pos)的协方差
  // 3到5行和列：旋转(rot)的协方差
  // 6到8行和列：相机与IMU之间的旋转偏移(offset_R_L_I)的协方差
  // 9到11行和列：相机与IMU之间的平移偏移(offset_T_L_I)的协方差
  // 12到14行和列：速度(vel)的协方差
  // 15到17行和列：陀螺仪偏差(bg)的协方差
  // 18到20行和列：加速度计偏差(ba)的协方差
  // 21到22行和列：重力(grav)的协方差
}
...
void ImuProcess::UndistortPcl(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI &pcl_out)
{
  ...
  for (auto it_imu = v_imu.begin(); it_imu < (v_imu.end() - 1); it_imu++)
  {
    ...
    // 输出预测值
    // cout << "pos: (" << imu_state.pos[0] << ", " << imu_state.pos[1] << ", " << imu_state.pos[2] << ")" << endl;
    // cout << "rot: (" << imu_state.rot.w() << ", " << imu_state.rot.x() << ", " << imu_state.rot.y() << ", " << imu_state.rot.z() << ")" << endl;
    out_imu.open(DEBUG_FILE_DIR("out_imu.txt"), ios::out | ios::app);
    out_imu << std::fixed << std::setprecision(6) << tail->header.stamp.toSec() << " " 
    << imu_state.pos[0] << " " << imu_state.pos[1] << " " << imu_state.pos[2] << " " 
    << imu_state.rot.x() << " " << imu_state.rot.y() << " " << imu_state.rot.z() << " " << imu_state.rot.w() << endl;
    out_imu.close();
  }
...
{
  double t1, t2, t3;
  t1 = omp_get_wtime();//从过去某个时间点开始到现在的经过的秒数。这个时间点是任意的，但是在程序执行期间保证不会改变。
  ...
  if (imu_need_init_)
  {
    ...
    state_ikfom imu_state = kf_state.get_x(); //更新完之后的机器人状态
    if (init_iter_num > MAX_INI_COUNT)
    {
      cov_acc *= pow(G_m_s2 / mean_acc.norm(), 2); // 将协方差加速度乘以重力加速度除以平均加速度的模的平方在上面IMU_init()基础上乘上缩放系数
